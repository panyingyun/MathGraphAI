...
from ..services.session_service import create_session, get_session_row, graph_to_json, session_schema, summary_schema, utc_now
...
def list_sessions(database: DatabaseSession = Depends(get_db)):
    rows = database.scalars(select(SessionModel).order_by(SessionModel.is_favorite.desc(), SessionModel.updated_at.desc())).all()
    ...
    if payload.graph_state is not None:
        try:
            for equation in payload.graph_state.equations:
                equation.normalized_expression = validate_expression(equation.normalized_expression or equation.expression)
        ...
        row.graph_state = graph_to_json(payload.graph_state)
