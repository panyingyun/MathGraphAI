...
> 状态：**已完成**（见 `docs/baseline/stage4.md`）

目标：在统一 ReAct 主链路稳定后，增加真正依赖 Observation 的数学工具，并通过 Shadow 模式验证执行质量。

任务：

- [x] 增加交点、零点、极值、函数比较、采样检查和视口拟合工具。
- [x] 工具 Observation 使用稳定 Schema、误差范围和大小限制。
- [x] 通过 `shadow` 同时运行新 Agent 决策但不提交，对比旧链路或基准答案。
- [x] 建立 Agent 轨迹回放测试，确保同一组模拟决策可以稳定重放。
- [x] Agent 失败时返回结构化错误；DeepSeek 不可用时切换 LocalDecisionProvider，而不是离开 Runner。
- [x] 对工具精度、循环终止、回滚、超时和重复调用进行专项测试。

首批工具增强场景：

- [x] 计算两条函数交点后自动调整视口。
- [x] 分析零点或极值后，在图上添加对应标记。
- [x] 根据采样结果判断函数在当前范围是否可绘制，再决定视口或提示用户。
- [x] 比较多条函数，并根据计算结果生成解释。
